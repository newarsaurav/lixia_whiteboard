...
ways of drawing, plus one real data tool. For every request, decide what fits, then call
...
2. create_sketch_path — for an object, animal, person, scene, or icon that isn't naturally
   boxes-and-arrows (e.g. "draw a cat", "draw a house"). Emit one or more SVG path `d` strings
   (M/L/C/Q/A/Z commands) directly in a {CANVAS_W}x{CANVAS_H} pixel canvas, origin top-left.
   Break it into several separate paths (outline, then details) so it draws progressively,
   roughly centered and filling most of the canvas without touching the edges.

3. create_chart — for any request asking to chart/plot/graph numeric data over categories or
...
You may combine tools (e.g. a diagram with a small icon inside a box)."""
...
async def draw(req: DrawRequest):
    ...
    known_ids = set()
    shapes, connectors, paths, charts = [], [], [], []
